...
public class VoronoiUtility : MonoBehaviour
{
    public Texture2D CreateDiagram(Vector2Int dimensions, Vector2Int[] centroids)
    {
        ...
        Vector2Int[] testC = new Vector2Int[centroids.Length];

        for (int i = 0; i < centroids.Length; i++)
        {
            regionColors[i] = new Color(Random.Range(0f, 1f), Random.Range(0f, 1f), Random.Range(0f, 1f), 1f);
            testC[i] = new Vector2Int(Random.Range(0, dimensions.x), Random.Range(0, dimensions.y));
        }
        ...
        for (int x = 0; x < dimensions.x; x++)
            for (int y = 0; y < dimensions.y; y++)
            {
                ...
                pixelColors[index] = regionColors[GetClosestCentroidIndex(new Vector2Int(x, y), testC)];
            }
